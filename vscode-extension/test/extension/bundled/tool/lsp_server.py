...
#     name="incc-lsp-displayname", version="<server version>"
# )
LSP_SERVER.run()
...
#     arr = ["TEST", "TES2", "TE3", "TE8"]
#     return [lsp.CompletionItem(label=x, kind=lsp.CompletionItemKind(18)) for x in arr]
#     return [lsp.CompletionItem(label=x) for x in arr]
...
def _run_tool_on_document(
    ...
    if use_path:
        ...
        with utils.substitute_attr(sys, "path", sys.path[:]):
            try:
                # TODO: `utils.run_module` is equivalent to running `python -m lsp-incc-vscode`.
